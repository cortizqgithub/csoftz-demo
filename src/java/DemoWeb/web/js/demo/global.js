...
/*-----------------------------------------------------------------------------
History
Jan.06/2011 COQ  File Created. 
------------------------------------------------------------------------------*/

function setupEnvironmentForGridPages() {
...
function setupEnvironmentForEditPages() {
    processingData('cmd');
}

function setupWithGreybox() {
    $('#Menu').tMenu();
    processingData('cmd');
...
function processingDataWithCheckboxesVerification(cmd) {
    $('#' + cmd).click(function () {
        ...
        if (l > 0) {
            var val = confirm('Are you sure you want to remove the selected records?');
            if (val) {
                $('#delRecsUsed').val('1');
                $.blockUI({ message: 'Processing. Hold on a moment please..' });
                $('form').submit();
